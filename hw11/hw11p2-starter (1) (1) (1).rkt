...
#reader(lib "htdp-intermediate-lambda-reader.ss" "lang")((modname |hw11p2-starter (1) (1)|) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
...
; Given a graph and a vertex, determines if no edge exists with the supplied vertex


(define (lonely? g n)
  (local [; lonely1? : [List-of Nat] -> Boolean
          ; Applies the edge function using the two items in the list
          (define (lonely1? lon)
            (apply (graph-e g) lon))
          ; notn  : [List-of Nat] Nat -> [List-of [List-of Nat]]
          ; Given a list of numbers and an n creates a list of pairs
          (define (notn lon n)
            (local [; func : Nat -> [List-of Nat]
                    ; Pairs a number with a given N
                    (define (func n1)
                      (list n1 n))]
              (map func (filter (lambda (y) (not (= y n))) lon))))]
    (andmap (lambda (t) (boolean=? t #false)) (map lonely1? (notn (graph-v g) n)))))
...
; is-tight? : Graph [List-of Nat]
; Given a Graph and a list of vertices determines if each vertice is the source of others


(define (is-tight? g lon)
  (local
    ; tight? : Nat -> Boolean
    ; Does the vertex connect with all other vertices?
    [(define (tight? v)
       (andmap (lambda (x) (or ((graph-e g) v x) (= v x))) lon))]
    (andmap tight? lon)))
...
; adjacency-matrix : Graph -> [List-of [List-of Nat]]
; producing the adjacency-matrix representation of a graph

(define (adjacency-matrix g)
  (local [; w : Nat Nat -> [List-of Nat]
          ; takes two numbers and pairs them in a list
          (define (w n1 n2)
            (list n1 n2))
          ; k : Nat -> [List-of Nat]
          ; 
          (define (k n)
            (map (lambda (w1) (w n w1)) (graph-v g)))
          (define (a lon)
            (apply (graph-e g) lon))
          (define (indi ln1)
            (map a ln1))
          (define (converter lob)
            (map (lambda (bl) (if (boolean=? bl #true) 1 0)) lob))]
    (map converter (map indi (map k (graph-v g))))))
